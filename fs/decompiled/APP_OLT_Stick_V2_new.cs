...
namespace APP_OLT_Stick_Eth
{
	public class CPE_Online : Form
	{
		...
		public void connect_SQL()
		{
			...
			try
			{
				...
				try
				{
					string text = "select * from " + sql_table + " where OLT_SN='nosnisrquired'";
					...
					try
					{
						...
						try
						{
							...
							((DbDataAdapter)(object)val3).Fill(dataTable_sql);
		...
		private int SaveFirstVisibleRow(DataGridView dgv)
		{
		...
		private void RestoreFirstVisibleRow(DataGridView dgv, int firstVisibleIndex)
		{
			if (firstVisibleIndex >= 0 && firstVisibleIndex < dgv.Rows.Count)
			{
				dgv.FirstDisplayedScrollingRowIndex = firstVisibleIndex;
			}
		}
		...
		private void timer_CPE_online_display_Tick(object sender, EventArgs e)
		{
			...
			try
			{
				int firstVisibleIndex = SaveFirstVisibleRow(dataGridView_ONU_Online_List);
				...
				if (oNU_STATUS_SN_List_temp != null)
				{
					...
					RestoreFirstVisibleRow(dataGridView_ONU_Online_List, firstVisibleIndex);
				}
			}
			catch
			{
	...
	public class Form1 : Form
	{
		...
		private ComboBox comboBox_net_ports;
		...
		private ToolStripMenuItem rEADACCESSSETToolStripMenuItem;

		private ToolStripMenuItem wRITEACCESSSETToolStripMenuItem;
		...
		public void init_udp_port()
		{
			using PortReserver portReserver = new PortReserver();
			if (!portReserver.ReservePort(64218))
			{
			}
		}
		...
		{
			dataGridView_OLT_Online_List.DefaultCellStyle = new DataGridViewCellStyle
			{
				Alignment = DataGridViewContentAlignment.MiddleCenter
			};
			dataGridView_OLT_Online_List.ColumnHeadersDefaultCellStyle = new DataGridViewCellStyle
			{
				Alignment = DataGridViewContentAlignment.MiddleCenter
			};
			...
			AddColumn("PORT", "switch_port", 120);
			AddColumn("ACCESS", "access_authority", 80);
			AddColumn("STAT", "olt_status", 80);
			AddColumn("TXPWR", "olt_tx_pwr", 80);
			AddColumn("BIAS", "olt_bias", 80);
			AddColumn("TEMP", "olt_temperature", 80);
			AddColumn("VOLT", "olt_voltage", 80);
			AddColumn("TYPE", "olt_pn", 80);
			AddColumn("ALARM", "alarm_status", 80);
			AddColumn("TIME", "time_string", 160);
			AddColumn("LAST", "offline_issue", 80);
		...
		private void Update_textBox_LOG_Management(string text)
		{
			...
			{
				if (textBox_LOG_Management.Text.Length > 51200)
				{
					logger.Log(textBox_LOG_Management.Text);
					textBox_LOG_Management.Text = "";
		...
		private int Send_Ethernet_Frame(byte[] sn, byte[] password, byte[] Frame_Content, PhysicalAddress physicalAddress, IPAddress iPAddress, ushort dest_udp_port)
		{
			int result = 0;
			try
			{
				manual_mAC.SendFrame(sn, password, Frame_Content, physicalAddress, iPAddress, dest_udp_port);
			}
			catch
			{
				result = -1;
		...
		private void send_cmd_upd_frame(string hexString, string Selected_OLT_address)
		{
			Frame_Sent_ID_Code_main++;
			byte[] array = ConvertStringToHexArray(hexString);
			byte b = 0;
			try
			{
			...
			case 64:
			{
				byte[] array4 = new byte[array.Length + 1];
				...
				try
				{
					...
					uDP_Analysis_Package.content = array4;
					...
					uDP_Analysis_Package.SenderProtocolAddress = IPAddress.Parse(Selected_OLT_address ?? "127.1.1.1");
					...
					if (num2 >= 0)
					{
						...
						uDP_Analysis_Package.source_UDP_port = oLT_Stick_Managements_List[num2].oLT_share_data.dest_UDP_port;
						byte[] array5 = new byte[oLT_Stick_Managements_List[num2].oLT_share_data.Vendor_Read_Key_byte.Length];
						Array.Copy(oLT_Stick_Managements_List[num2].oLT_share_data.Vendor_Read_Key_byte, array5, array5.Length);
						text2 = oLT_Stick_Managements_List[num2].oLT_share_data.OLT_SN;
						uDP_Analysis_Package.cmd_Code = (Command_Code)array4[0];
						uDP_Analysis_Package.upd_ID = upd_ID2;
						Send_Package_List.Add(uDP_Analysis_Package);
						if (Send_Ethernet_Frame(Encoding.UTF8.GetBytes(text2), array5, array4, uDP_Analysis_Package.SenderHardwareAddress, uDP_Analysis_Package.SenderProtocolAddress, uDP_Analysis_Package.source_UDP_port) != 0)
						{
			...
			case 65:
			{
				...
				try
				{
					...
					uDP_Analysis_Package2.SenderProtocolAddress = IPAddress.Parse(Selected_OLT_address ?? "");
					...
					if (num >= 0)
					{
						...
						uDP_Analysis_Package2.source_UDP_port = oLT_Stick_Managements_List[num].oLT_share_data.dest_UDP_port;
						array3 = new byte[oLT_Stick_Managements_List[num].oLT_share_data.Vendor_Read_Key_byte.Length];
						Array.Copy(oLT_Stick_Managements_List[num].oLT_share_data.Vendor_Read_Key_byte, array3, array3.Length);
						text = oLT_Stick_Managements_List[num].oLT_share_data.OLT_SN;
						...
						if (Send_Ethernet_Frame(Encoding.UTF8.GetBytes(text), array3, array2, uDP_Analysis_Package2.SenderHardwareAddress, uDP_Analysis_Package2.SenderProtocolAddress, uDP_Analysis_Package2.source_UDP_port) != 0)
						{
							MessageBox.Show("cannot connect with OLT！");
						}
					}
		...
		private void button_Send_MAC_Frame_Click(object sender, EventArgs e)
		{
			...
			{
				UpdateTextBox("Please select OLT_Stick ip address......");
				return;
			}
			text2 = comboBox_OLT_stick_ADD.SelectedValue?.ToString();
			send_cmd_upd_frame(text, text2);
		}
		...
		public string ExtractOLTSerialNumberRegex(string input)
		{
			Match match = Regex.Match(input, "oltiSN:(.{16})");
			if (match.Success)
			{
		...
		private void timer_Receiver_Message_Process_Tick(object sender, EventArgs e)
		{
			...
			for (int k = 0; k < num; k++)
			{
				...
				try
				{
					...
					ushort source_UDP_port = (ushort)(list[k].Content[28] * 256 + list[k].Content[29]);
					uDP_Analysis_Package.source_UDP_port = source_UDP_port;
					uDP_Analysis_Package.cmd_Code = (Command_Code)list[k].Content[52];
					uDP_Analysis_Package.upd_ID = (ushort)(list[k].Content[53] * 256 + list[k].Content[54]);
					Array.Copy(list[k].Content, 36, uDP_Analysis_Package.olt_sn, 0, 16);
					byte[] content = new byte[1600];
					uDP_Analysis_Package.content = content;
					Array.Copy(list[k].Content, 52, uDP_Analysis_Package.content, 0, list[k].Content.Length - 52);
					uDP_Analysis_Package.aging_time = (DateTime.Now - new DateTime(1970, 1, 1)).TotalSeconds;
					uDP_Analysis_Package.time_window = 60.0;
					uDP_Analysis_Package.UDP_package_length = (ushort)(list[k].Content[32] * 256 + list[k].Content[33]) - 8 - 16;
				}
				catch
				{
				}
				lock (Lock_oLT_Stick_Managements_List)
				{
					...
					{
						oLT_Stick_Management.oLT_share_data.iPAddress = uDP_Analysis_Package.SenderProtocolAddress;
						oLT_Stick_Management.oLT_share_data.physicalAddress = uDP_Analysis_Package.SenderHardwareAddress;
						oLT_Stick_Management.oLT_share_data.dest_UDP_port = uDP_Analysis_Package.source_UDP_port;
						switch (uDP_Analysis_Package.cmd_Code)
						{
						case Command_Code.shake_hand:
							oLT_Stick_Management.oLT_share_data.age_time = 1200;
							UpdateTextBox("shake_hand-" + uDP_Analysis_Package.SenderProtocolAddress.ToString());
						...
						case Command_Code.OLT_login_send:
						case Command_Code.OLT_Status_report:
						case Command_Code.OLT_Alarm_report:
						case Command_Code.ONU_Status_report:
						case Command_Code.ONU_Alarm_report:
							try
							{
								byte[] destinationArray = new byte[uDP_Analysis_Package.UDP_package_length - 1];
								byte[] array3 = new byte[uDP_Analysis_Package.UDP_package_length - 2];
								Array.Copy(uDP_Analysis_Package.content, 3, destinationArray, 0, uDP_Analysis_Package.UDP_package_length - 1 - 2);
								Array.Copy(uDP_Analysis_Package.content, 4, array3, 0, uDP_Analysis_Package.UDP_package_length - 2 - 2);
								string @string = Encoding.ASCII.GetString(array3);
								Textlog_With_Time(iPAddress.ToString() + "-" + @string);
								if (@string.Contains("dying_gasp"))
								{
									string onusn = ExtractSerialNumberRegex(@string);
									Speical_status_onu_sn_ONU_Monitor(iPAddress.ToString(), onusn, "DYING_GASP");
								}
								else if (@string.Contains("on_line"))
								{
									string onusn2 = ExtractSerialNumberRegex(@string);
									Speical_status_onu_sn_ONU_Monitor(iPAddress.ToString(), onusn2, "ON_LINE");
								}
								else if (@string.Contains("off_line"))
								{
									string onusn3 = ExtractSerialNumberRegex(@string);
									Speical_status_onu_sn_ONU_Monitor(iPAddress.ToString(), onusn3, "OFF_LINE");
								}
								else if (@string.Contains("reset_ok"))
								{
									string onusn4 = ExtractSerialNumberRegex(@string);
									Speical_status_onu_sn_ONU_Monitor(iPAddress.ToString(), onusn4, "CONFIGURE");
								}
								else if (@string.Contains("upload_ok"))
								{
									string onusn5 = ExtractSerialNumberRegex(@string);
									Speical_status_onu_sn_ONU_Monitor(iPAddress.ToString(), onusn5, "UP_LOAD");
								}
								else if (@string.Contains("LOSLOFalarm"))
								{
									string onusn6 = ExtractSerialNumberRegex(@string);
									Speical_status_onu_sn_ONU_Monitor(iPAddress.ToString(), onusn6, "LOFi");
								}
								else if (@string.Contains("Rogue_ONU_Alarm"))
								{
									string oltsn = ExtractSerialNumberRegex(@string);
									Speical_status_OLT_sn_OLT_Monitor(iPAddress, oltsn, "Rogue_ONU_Alarm");
								}
								else if (@string.Contains("oltiSN"))
								{
									string sn = ExtractOLTSerialNumberRegex(@string);
									olt_add_basedon_oltinfo_report(@string, sn, uDP_Analysis_Package);
								}
								else if (@string.Contains("Neighbor"))
								{
									string pattern = "^(?:[^|]*\\|){4}([^|]*)\\|([^|]*)";
									...
									if (match.Success)
									{
										...
										olt_info_update_port(value, uDP_Analysis_Package);
									}
								}
								else if (@string.Contains("Get PN:"))
								{
									olt_pn_report(@string, uDP_Analysis_Package);
								}
								else if (@string.Contains("access result"))
								{
									olt_password_access_report(@string, uDP_Analysis_Package);
								}
								else if (uDP_Analysis_Package.cmd_Code == Command_Code.ONU_Status_report)
								{
					...
					else if (uDP_Analysis_Package.cmd_Code == Command_Code.OLT_login_send)
					{
						byte[] destinationArray2 = new byte[uDP_Analysis_Package.UDP_package_length - 1];
						byte[] array4 = new byte[uDP_Analysis_Package.UDP_package_length - 2];
						Array.Copy(uDP_Analysis_Package.content, 3, destinationArray2, 0, uDP_Analysis_Package.UDP_package_length - 1 - 2);
						Array.Copy(uDP_Analysis_Package.content, 4, array4, 0, uDP_Analysis_Package.UDP_package_length - 2 - 2);
						string string2 = Encoding.ASCII.GetString(array4);
						Textlog_With_Time(iPAddress.ToString() + "-" + string2);
						if (string2.Contains("oltiSN"))
						{
							string sn2 = ExtractOLTSerialNumberRegex(string2);
							olt_add_basedon_oltinfo_report(string2, sn2, uDP_Analysis_Package);
						}
					}
				}
			}
			...
			for (i = 0; i < num; i++)
			{
				...
				string text = copyArpCircularBuffer[i].ToString();
				...
				if (copyArpCircularBuffer[i].Opcode == 1)
				{
					lock (Lock_oLT_Stick_Managements_List)
					{
						...
						try
						{
							if (!copyArpCircularBuffer[i].specified_code.Contains("OLT"))
							{
								return;
							}
							OLT_Stick_Management oLT_Stick_Management3 = oLT_Stick_Managements_List.FirstOrDefault((OLT_Stick_Management olt) => olt.oLT_share_data.OLT_SN.Contains(copyArpCircularBuffer[i].specified_code.Substring(3)));
							if (oLT_Stick_Management3 != null)
							{
								oLT_Stick_Management3.oLT_share_data.iPAddress = copyArpCircularBuffer[i].SenderProtocolAddress;
								oLT_Stick_Management3.oLT_share_data.active = true;
								oLT_Stick_Management3.oLT_share_data.Comment = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
								oLT_Stick_Management3.oLT_share_data.age_time = 0;
								oLT_Stick_Management3.oLT_share_data.Vendor_Write_Key_byte = Encoding.UTF8.GetBytes(Vendor_Write_Key);
								oLT_Stick_Management3.oLT_share_data.Vendor_Read_Key_byte = Encoding.UTF8.GetBytes(Vendor_Read_Key);
								Update_IP_OLT_ipaddress(oLT_Stick_Management3);
							}
							else if (copyArpCircularBuffer[i].specified_code.Contains("OLT"))
							{
								UpdateTextBox($"{i + 1}arp:" + text);
								...
								oLT_share_data.OLT_SN = copyArpCircularBuffer[i].specified_code.Substring(3);
								oLT_share_data.Comment = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
								oLT_share_data.Vendor_Write_Key_byte = Encoding.UTF8.GetBytes(Vendor_Write_Key);
								oLT_share_data.Vendor_Read_Key_byte = Encoding.UTF8.GetBytes(Vendor_Read_Key);
								OLT_Stick_Management oLT_Stick_Management4 = new OLT_Stick_Management(oLT_share_data, oLT_Stick_Managements_List.Count + 1);
								oLT_Stick_Management4.oLT_share_data.age_time = 0;
								oLT_Stick_Management4.oLT_share_data.active = true;
								lock (Lock_oLT_Stick_Managements_List)
								{
									oLT_Stick_Managements_List.Add(oLT_Stick_Management4);
									Adding_OLT_AppData_user(oLT_Stick_Management4);
								}
							}
						}
						catch
				...
				else
				{
					...
					lock (Lock_oLT_Stick_Managements_List)
					{
						OLT_Stick_Management oLT_Stick_Management5 = oLT_Stick_Managements_List.FirstOrDefault((OLT_Stick_Management olt) => olt.oLT_share_data.iPAddress.Equals(copyArpCircularBuffer[i].SenderProtocolAddress));
						if (oLT_Stick_Management5 != null)
						{
							oLT_Stick_Management5.oLT_share_data.age_time = 1500;
							oLT_Stick_Management5.oLT_share_data.physicalAddress = copyArpCircularBuffer[i].SenderHardwareAddress;
						}
					}
				}
			}
		...
		private void Speical_status_OLT_sn_OLT_Monitor(IPAddress oltip, string oltsn, string alarm)
		{
			IPAddress oltip2 = oltip;
			lock (Lock_oLT_Stick_Managements_List)
			{
				OLT_Stick_Management oLT_Stick_Management = oLT_Stick_Managements_List.FirstOrDefault((OLT_Stick_Management olt) => olt.oLT_share_data.iPAddress.Equals(oltip2));
				if (oLT_Stick_Management == null)
				{
					return;
				}
				...
				try
				{
					OLT_Performance_Notity oLT_Performance_Notity = OLT_AppData.Users.FirstOrDefault((OLT_Performance_Notity olt) => olt != null && olt.olt_ip != null && olt.olt_ip.Equals(oltip2));
					oLT_Performance_Notity.alarm_status = alarm.ToString();
		...
		private void olt_pn_report(string frame_content, UDP_Analysis_Package uDP_Analysis_Package)
		{
			UDP_Analysis_Package uDP_Analysis_Package2 = uDP_Analysis_Package;
			try
			{
				string pattern = ":\\s*(.+)";
				Match match = Regex.Match(frame_content, pattern);
				if (!match.Success)
				{
					return;
				}
				...
				lock (Lock_oLT_Stick_Managements_List)
				{
					OLT_Stick_Management oLT_Stick_Management = oLT_Stick_Managements_List.FirstOrDefault((OLT_Stick_Management olt) => olt.oLT_share_data.iPAddress.Equals(uDP_Analysis_Package2.SenderProtocolAddress));
					if (oLT_Stick_Management != null)
					{
						OLT_Performance_Notity oLT_Performance_Notity = OLT_AppData.Users.FirstOrDefault((OLT_Performance_Notity olt) => olt != null && olt.olt_ip != null && olt.olt_ip.Equals(uDP_Analysis_Package2.SenderProtocolAddress));
						if (oLT_Performance_Notity != null)
						{
							oLT_Performance_Notity.olt_pn = value;
						}
					}
				}
			}
		...
		private void olt_password_access_report(string frame_content, UDP_Analysis_Package uDP_Analysis_Package)
		{
			UDP_Analysis_Package uDP_Analysis_Package2 = uDP_Analysis_Package;
			try
			{
				string pattern = "R=(?<R>\\d+);RCNT=(?<RCNT>\\d+);W=(?<W>\\d+);WCNT=(?<WCNT>\\d+)";
				Match match = Regex.Match(frame_content, pattern);
				if (!match.Success)
				{
					return;
				}
				int num = int.Parse(match.Groups["R"].Value);
				int olt_read_access_tick = int.Parse(match.Groups["RCNT"].Value);
				int num2 = int.Parse(match.Groups["W"].Value);
				int olt_write_access_tick = int.Parse(match.Groups["WCNT"].Value);
				lock (Lock_oLT_Stick_Managements_List)
				{
					OLT_Stick_Management oLT_Stick_Management = oLT_Stick_Managements_List.FirstOrDefault((OLT_Stick_Management olt) => olt.oLT_share_data.iPAddress.Equals(uDP_Analysis_Package2.SenderProtocolAddress));
					if (oLT_Stick_Management == null)
					...
					oLT_Stick_Management.oLT_share_data.olt_read_access_flag = num;
					oLT_Stick_Management.oLT_share_data.olt_read_access_tick = olt_read_access_tick;
					oLT_Stick_Management.oLT_share_data.olt_write_access_flag = num2;
					oLT_Stick_Management.oLT_share_data.olt_write_access_tick = olt_write_access_tick;
					OLT_Performance_Notity oLT_Performance_Notity = OLT_AppData.Users.FirstOrDefault((OLT_Performance_Notity olt) => olt != null && olt.olt_ip != null && olt.olt_ip.Equals(uDP_Analysis_Package2.SenderProtocolAddress));
					if (oLT_Performance_Notity != null)
					{
						if (num == 1 && num2 == 1)
						{
							oLT_Performance_Notity.access_authority = "R/W";
						}
						else if (num == 1)
						{
							oLT_Performance_Notity.access_authority = "R/";
						}
						else if (num2 == 1)
						{
							oLT_Performance_Notity.access_authority = "/W";
						}
						else
						{
							oLT_Performance_Notity.access_authority = "/";
						}
					}
				}
			}
		...
		private void olt_info_update_port(string port, UDP_Analysis_Package uDP_Analysis_Package)
		{
			UDP_Analysis_Package uDP_Analysis_Package2 = uDP_Analysis_Package;
			try
			{
				lock (Lock_oLT_Stick_Managements_List)
				{
					...
					if (oLT_Stick_Management != null)
					{
						oLT_Stick_Management.oLT_share_data.sw_port = port;
						OLT_Performance_Notity oLT_Performance_Notity = OLT_AppData.Users.FirstOrDefault((OLT_Performance_Notity olt) => olt != null && olt.olt_ip != null && olt.olt_ip.Equals(uDP_Analysis_Package2.SenderProtocolAddress));
						if (oLT_Performance_Notity != null)
						{
							oLT_Performance_Notity.switch_port = port;
						}
					}
				}
			}
		...
		private void olt_add_basedon_oltinfo_report(string frame_content, string sn, UDP_Analysis_Package uDP_Analysis_Package)
		{
			UDP_Analysis_Package uDP_Analysis_Package2 = uDP_Analysis_Package;
			string sn2 = sn;
			lock (Lock_oLT_Stick_Managements_List)
			{
				...
				if (oLT_Stick_Management != null)
				{
					...
					try
					{
						(float voltage, float temperature, float bias, float tx_pwr, uint alarm, string pn_result) tuple = ParseOltParameters(frame_content);
						float item = tuple.voltage;
						float item2 = tuple.temperature;
						float item3 = tuple.bias;
						float item4 = tuple.tx_pwr;
						uint item5 = tuple.alarm;
						string item6 = tuple.pn_result;
						OLT_Performance_Notity oLT_Performance_Notity = OLT_AppData.Users.FirstOrDefault((OLT_Performance_Notity olt) => olt != null && olt.olt_ip != null && olt.olt_ip.Equals(uDP_Analysis_Package2.SenderProtocolAddress));
						oLT_Performance_Notity.olt_status = oLT_Stick_Management.oLT_share_data.Current_status;
						oLT_Performance_Notity.olt_temperature = item2.ToString();
						oLT_Performance_Notity.olt_voltage = item.ToString();
						oLT_Performance_Notity.olt_tx_pwr = item4.ToString("F2");
						oLT_Performance_Notity.olt_bias = item3.ToString();
						oLT_Performance_Notity.olt_sn = sn2;
						oLT_Performance_Notity.time_second = (DateTime.Now - new DateTime(1970, 1, 1)).TotalSeconds;
						oLT_Performance_Notity.time_string = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
						oLT_Performance_Notity.olt_pn = item6;
						return;
					}
					catch
					{
				...
				try
				{
					OLT_Stick_Management oLT_Stick_Management2 = oLT_Stick_Managements_List.FirstOrDefault((OLT_Stick_Management olt) => olt.oLT_share_data.OLT_SN.Contains(sn2));
					if (oLT_Stick_Management2 != null)
					{
						oLT_Stick_Management2.oLT_share_data.iPAddress = uDP_Analysis_Package2.SenderProtocolAddress;
						oLT_Stick_Management2.oLT_share_data.active = true;
						oLT_Stick_Management2.oLT_share_data.Comment = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
						oLT_Stick_Management2.oLT_share_data.age_time = 0;
						oLT_Stick_Management2.oLT_share_data.Vendor_Write_Key_byte = Encoding.UTF8.GetBytes(Vendor_Write_Key);
						oLT_Stick_Management2.oLT_share_data.Vendor_Read_Key_byte = Encoding.UTF8.GetBytes(Vendor_Read_Key);
						Update_IP_OLT_ipaddress(oLT_Stick_Management2);
						return;
					}
					OLT_share_data oLT_share_data = new OLT_share_data();
					oLT_share_data.iPAddress = uDP_Analysis_Package2.SenderProtocolAddress;
					oLT_share_data.physicalAddress = uDP_Analysis_Package2.SenderHardwareAddress;
					oLT_share_data.OLT_SN = sn2;
					oLT_share_data.Comment = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
					oLT_share_data.Vendor_Write_Key_byte = Encoding.UTF8.GetBytes(Vendor_Write_Key);
					oLT_share_data.Vendor_Read_Key_byte = Encoding.UTF8.GetBytes(Vendor_Read_Key);
					...
					lock (Lock_oLT_Stick_Managements_List)
					{
						...
						var (voltage, temperature, bias, tx_pwr, alarm, pn_result) = ParseOltParameters(frame_content);
						Adding_OLT_AppData_user(oLT_Stick_Management3, voltage, temperature, bias, tx_pwr, alarm, pn_result);
					}
				}
				catch
				{
		...
		private void Adding_OLT_AppData_user(OLT_Stick_Management temp, float voltage, float temperature, float bias, float tx_pwr, uint alarm, string pn_result)
		{
			...
			oLT_Performance_Notity.time_second = (DateTime.Now - new DateTime(1970, 1, 1)).TotalSeconds;
			oLT_Performance_Notity.time_string = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
			oLT_Performance_Notity.olt_status = "ON_LINE";
			oLT_Performance_Notity.alarm_status = alarm.ToString();
			oLT_Performance_Notity.olt_tx_pwr = tx_pwr.ToString("f2");
			oLT_Performance_Notity.olt_temperature = temperature.ToString("f2");
			oLT_Performance_Notity.olt_bias = bias.ToString("f2");
			oLT_Performance_Notity.olt_voltage = voltage.ToString("f2");
			oLT_Performance_Notity.olt_pn = pn_result;
		...
		{
			OLT_Stick_Management working_OLT2 = working_OLT;
			try
			{
				lock (MacSendFrameGlobalLock)
				{
					working_OLT2.shake_Hands.shake_hands_fun(mAC);
					Update_IP_OLT_Monitor(working_OLT2);
					OLT_Performance_Notity oLT_Performance_Notity = OLT_AppData.Users.FirstOrDefault((OLT_Performance_Notity olt) => olt != null && olt.olt_ip != null && olt.olt_ip.Equals(working_OLT2.oLT_share_data.iPAddress));
					if (oLT_Performance_Notity != null)
					{
						oLT_Performance_Notity.olt_status = working_OLT2.oLT_share_data.Current_status;
					}
				}
			}
			catch
				...
				try
				{
					foreach (OLT_Stick_Management working_OLT in oLT_Stick_Managements_List)
					{
						...
						if (Active)
						{
							...
							if (working_OLT.oLT_share_data.olt_stick_Lifecycle == 1 && working_OLT.oLT_share_data.olt_read_access_flag > 0)
							{
							...
							if (working_OLT.oLT_share_data.olt_stick_Lifecycle == 1 && working_OLT.oLT_share_data.olt_write_access_flag > 0)
							{
								await Task.Delay(500);
			...
			if (folderBrowserDialog.ShowDialog() == DialogResult.OK)
			{
				...
				for (int i = 0; i < oNU_Service_Type_Info_Stru.oNU_Service_Type_Info_List.Count; i++)
				{
					...
					for (int j = 0; j < 5; j++)
					{
						...
						ushort vlan1_id = oNU_Service_Type_Info_Stru.oNU_Service_Type_Info_List[i].oNU_Service_Flows[j].vlan1_id;
						ushort vlan2_id = oNU_Service_Type_Info_Stru.oNU_Service_Type_Info_List[i].oNU_Service_Flows[j].vlan2_id;
						ushort vlan3_id = oNU_Service_Type_Info_Stru.oNU_Service_Type_Info_List[i].oNU_Service_Flows[j].vlan3_id;
						ushort vlan4_id = oNU_Service_Type_Info_Stru.oNU_Service_Type_Info_List[i].oNU_Service_Flows[j].vlan4_id;
						ushort vlan5_id = oNU_Service_Type_Info_Stru.oNU_Service_Type_Info_List[i].oNU_Service_Flows[j].vlan5_id;
						text3 = text3 + $"<flow {j + 1} start>" + Environment.NewLine;
						text3 = text3 + $"     >max_BD   {max}                   //uint16_t, max bandwith , Unit 20Kbit/s" + Environment.NewLine;
						text3 = text3 + $"     >fix_BD   {fix}                   //uint16_t, fixed bandwith , Unit 20Kbit/s" + Environment.NewLine;
						text3 = text3 + $"     >ass_BD   {ass}                   //uint16_t, assure bandwith  ,Unit 20Kbit/s" + Environment.NewLine;
						text3 = text3 + $"     >type   {type}                   //uint8_t, service type 1~5" + Environment.NewLine;
						text3 = text3 + $"     >priority   {priority}                   //uint8_t, priority, 0~7,  0: top priority" + Environment.NewLine;
						text3 = text3 + $"     >weight   {weight}                   //uint8_t, range, 1~255, 255: top weight" + Environment.NewLine;
						text3 = text3 + $"     >valid   {valid}                   //1: valid; 0: invalid" + Environment.NewLine;
						text3 = text3 + $"     >vlan1_id   {vlan1_id}                   //uint16_t, pptep1  VLAN(TCI), 0: no vlan is required" + Environment.NewLine;
						text3 = text3 + $"     >vlan2_id   {vlan2_id}                   //uint16_t, pptep2  VLAN(TCI)" + Environment.NewLine;
						text3 = text3 + $"     >vlan3_id   {vlan3_id}                   //uint16_t, pptep3  VLAN(TCI)" + Environment.NewLine;
						text3 = text3 + $"     >vlan4_id   {vlan4_id}                   //uint16_t, pptep4  VLAN(TCI)" + Environment.NewLine;
						text3 = text3 + $"     >vlan5_id   {vlan5_id}                   //uint16_t, pptep5  VLAN(TCI)" + Environment.NewLine;
			...
			try
			{
				lock (MacSendFrameGlobalLock)
				{
					...
					if (num == 1)
					{
						...
						{
							((DbConnection)(object)val).Open();
							string text = $"update OLT_Information_List set IP_Add = '{result.OLT_IP_NEW.ToString()}' WHERE IP_Add = '{result.OLT_IP_OLD.ToString()}'";
							SQLiteCommand val2 = new SQLiteCommand(text, val);
							try
							{
		...
		private void pwd_write(OLT_Stick_Management working_OLT)
		{
			...
			try
			{
				...
				{
					int num = 0;
					if (working_OLT2.pWD_Set.change_W == 87)
					{
						working_OLT2.pWD_Set.Vendor_Write_Key = Vendor_Write_Key;
						int num2 = working_OLT2.pWD_Set.PWD_Change(mAC, flag: true, 1);
						working_OLT2.pWD_Set.change_W = 0;
						working_OLT2.oLT_share_data.last_read_time = 0.0;
						if (num2 == 0)
						{
							num = 1;
						}
					}
					else if (working_OLT2.pWD_Set.change_R == 82)
					{
						working_OLT2.pWD_Set.Vendor_Read_Key = Vendor_Read_Key;
						int num3 = working_OLT2.pWD_Set.PWD_Change(mAC, flag: true, 2);
						working_OLT2.pWD_Set.change_R = 0;
						working_OLT2.oLT_share_data.last_read_time = 0.0;
						if (num3 == 0)
						{
							num = 2;
					...
					if (num != 0)
					{
						if (num == 2)
						{
							working_OLT2.oLT_share_data.olt_read_access_flag = 1;
							working_OLT2.oLT_share_data.olt_read_access_tick = 0;
						}
						if (num == 1)
						{
							working_OLT2.oLT_share_data.olt_write_access_flag = 1;
							working_OLT2.oLT_share_data.olt_write_access_tick = 0;
						}
						OLT_Performance_Notity oLT_Performance_Notity = OLT_AppData.Users.FirstOrDefault((OLT_Performance_Notity olt) => olt != null && olt.olt_ip != null && olt.olt_ip.Equals(working_OLT2.oLT_share_data.iPAddress));
						if (oLT_Performance_Notity != null)
						{
							if (working_OLT2.oLT_share_data.olt_read_access_flag == 1 && working_OLT2.oLT_share_data.olt_write_access_flag == 1)
							{
								oLT_Performance_Notity.access_authority = "R/W";
							}
							else if (working_OLT2.oLT_share_data.olt_read_access_flag == 1)
							{
								oLT_Performance_Notity.access_authority = "R/";
							}
							else if (working_OLT2.oLT_share_data.olt_write_access_flag == 1)
							{
								oLT_Performance_Notity.access_authority = "/W";
							}
							else
					...
					working_OLT2.shake_Hands.last_reading_package_time = (DateTime.Now - new DateTime(1970, 1, 1)).TotalSeconds;
				}
			}
			catch
			{
		...
		private void deleteToolStripMenuItem_Click(object sender, EventArgs e)
		{
			...
			try
			{
				...
				{
					((DbConnection)(object)val).Open();
					string text2 = "delete from OLT_Information_List WHERE IP_Add = '" + text + "'";
		...
		private void Update_IP_OLT_ipaddress(OLT_Stick_Management olt_instant)
		{
			OLT_Stick_Management olt_instant2 = olt_instant;
			try
			{
				OLT_Performance_Notity oLT_Performance_Notity = OLT_AppData.Users.FirstOrDefault((OLT_Performance_Notity olt) => olt != null && olt.olt_ip != null && olt.olt_sn == olt_instant2.oLT_share_data.OLT_SN);
				if (oLT_Performance_Notity != null)
				{
					if (oLT_Performance_Notity.olt_status != olt_instant2.oLT_share_data.Current_status)
					{
					...
					oLT_Performance_Notity.olt_status = olt_instant2.oLT_share_data.Current_status;
					oLT_Performance_Notity.olt_ip = olt_instant2.oLT_share_data.iPAddress;
					oLT_Performance_Notity.time_second = (DateTime.Now - new DateTime(1970, 1, 1)).TotalSeconds;
					oLT_Performance_Notity.time_string = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
				}
			}
		...
		{
			string input = "get_onu_optics(\"" + onusn + "\")";
			byte[] array = ConvertStringToHexArray(input);
			byte b = 65;
			int num = 0;
			if (b == 65)
			{
				...
				try
				{
					...
					uDP_Analysis_Package.SenderHardwareAddress = oLT_Stick_Managements_List[num2].oLT_share_data.physicalAddress;
					uDP_Analysis_Package.source_UDP_port = oLT_Stick_Managements_List[num2].oLT_share_data.dest_UDP_port;
					byte[] array3 = new byte[oLT_Stick_Managements_List[num2].oLT_share_data.Vendor_Read_Key_byte.Length];
					Array.Copy(oLT_Stick_Managements_List[num2].oLT_share_data.Vendor_Read_Key_byte, array3, array3.Length);
					text = oLT_Stick_Managements_List[num2].oLT_share_data.OLT_SN;
					uDP_Analysis_Package.cmd_Code = (Command_Code)array2[0];
					uDP_Analysis_Package.upd_ID = upd_ID;
					Send_Package_List.Add(uDP_Analysis_Package);
					try
					{
						mymAC.SendFrame(Encoding.UTF8.GetBytes(text), array3, array2, uDP_Analysis_Package.SenderHardwareAddress, uDP_Analysis_Package.SenderProtocolAddress, uDP_Analysis_Package.source_UDP_port);
					}
					catch
					{
						num = -1;
		...
		private static uint ExtractInt(Regex regex, string input)
		{
			Match match = regex.Match(input);
			if (match.Success && uint.TryParse(match.Value, out var result))
			{
				return result;
			}
			return 0u;
		...
		public static (float voltage, float temperature, float bias, float tx_pwr, uint alarm, string pn_result) ParseOltParameters(string input)
		{
			float item = 0f;
			float item2 = 0f;
			float item3 = 0f;
			...
			if (array.Length >= 6)
			{
				...
				item4 = (float)(10.0 * Math.Log10(ExtractFloat(regex, array[3])));
				item3 = ExtractFloat(regex, array[4]);
				item = ExtractFloat(regex, array[5]);
				item2 = ExtractFloat(regex, array[6]);
				item5 = ExtractInt(regex, array[7]);
			}
			string pattern = "pn=([^;]*)";
			Match match = Regex.Match(input, pattern);
			if (match.Success)
			{
				item6 = match.Groups[1].Value.Trim();
			}
			return (voltage: item, temperature: item2, bias: item3, tx_pwr: item4, alarm: item5, pn_result: item6);
		}
			...
			try
			{
				while (!uDP_Analysis_Packages_CMD0x41.IsEmpty)
				{
					if (uDP_Analysis_Packages_CMD0x41.TryDequeue(out UDP_Analysis_Package result) && result != null)
					{
						...
						Array.Copy(result.content, 3, array, 0, result.UDP_package_length - 1 - 2);
						Array.Copy(result.content, 4, array2, 0, result.UDP_package_length - 2 - 2);
						if (array[0] == 65)
						{
						...
						try
						{
							...
							else
							{
								ONU_Performance_Notity newONU = new ONU_Performance_Notity
								{
									olt_ip = result.SenderProtocolAddress,
									onu_sn = (onusn ?? ""),
									onu_status = "ON_LINE",
									onu_rx_pwr = item5.ToString("F2"),
									onu_tx_pwr = item4.ToString("F2"),
									onu_temperature = item2.ToString("F2"),
									onu_bias = item3.ToString("F2"),
									time_second = (DateTime.Now - new DateTime(1970, 1, 1)).TotalSeconds,
									time_string = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"),
									needsaverightnow = "YES"
								};
		...
		private void timer_ONU_optics_read_Tick(object sender, EventArgs e)
		{
					...
					foreach (List<ONU_Performance_Notity> item in AppData.GetGroupsByOLT())
					{
						foreach (ONU_Performance_Notity item2 in item)
						{
							...
							if (oNU_Performance_Notity != null && (DateTime.Now - new DateTime(1970, 1, 1)).TotalSeconds - oNU_Performance_Notity.time_second > 20.0)
							{
								...
								if (num != -1 && oLT_Stick_Managements_List[num].oLT_share_data.Current_status == "ON_LINE" && oLT_Stick_Managements_List[num].oLT_share_data.sql_station != "downloadFW" && oLT_Stick_Managements_List[num].oLT_share_data.sql_station != "WhlstSet" && oLT_Stick_Managements_List[num].oLT_share_data.olt_read_access_flag > 0)
								{
			...
			try
			{
				...
				foreach (OLT_Stick_Management oLT_Stick_Managements_ in oLT_Stick_Managements_List)
				{
					if (oLT_Stick_Managements_.oLT_share_data.active && (DateTime.Now - new DateTime(1970, 1, 1)).TotalSeconds - oLT_Stick_Managements_.oLT_share_data.last_read_time > 20.0)
					{
						oLT_Stick_Managements_.oLT_share_data.last_read_time = (DateTime.Now - new DateTime(1970, 1, 1)).TotalSeconds;
						send_cmd_upd_frame("get_port_info()", oLT_Stick_Managements_.oLT_share_data.iPAddress.ToString());
						send_cmd_upd_frame("get_access_status()", oLT_Stick_Managements_.oLT_share_data.iPAddress.ToString());
					}
				}
			}
			catch
		...
		private void timer_Performace_save_Tick(object sender, EventArgs e)
		{
			...
			if (_runningTask == null || _runningTask.IsCompleted || _runningTask.IsFaulted || _runningTask.IsCanceled)
			{
				Textlog_With_Time("olt polling is stopped, restarting...");
				StartProcessingTask();
			}
		}

		private void rEADACCESSSETToolStripMenuItem_Click(object sender, EventArgs e)
		{
			contextMenuStrip_OLT_Check.Close();
			bool flag = true;
			try
			...
			try
			{
				...
				lock (Lock_oLT_Stick_Managements_List)
				{
					...
					if (num2 != -1)
					{
						if (oLT_Stick_Managements_List[num2].oLT_share_data.status_int == 0)
						{
							Textlog_With_Time("cannot set read access in OLT IP_Add=" + comment + ", since it's not active");
						}
						else
						{
							oLT_Stick_Managements_List[num2].pWD_Set.change_R = 82;
						}
					}
					else
					{
		...
		private void wRITEACCESSSETToolStripMenuItem_Click(object sender, EventArgs e)
		{
			contextMenuStrip_OLT_Check.Close();
			bool flag = true;
			try
			...
			try
			{
				...
				lock (Lock_oLT_Stick_Managements_List)
				{
					...
					if (num2 != -1)
					{
						if (oLT_Stick_Managements_List[num2].oLT_share_data.status_int == 0)
						{
							Textlog_With_Time("cannot set write access in OLT IP_Add=" + comment + ", since it's not active");
						}
						else
						{
							oLT_Stick_Managements_List[num2].pWD_Set.change_W = 87;
						}
					}
					else
					{
		...
		private void InitializeComponent()
		{
			...
			this.comboBox_net_ports = new System.Windows.Forms.ComboBox();
			...
			this.rEADACCESSSETToolStripMenuItem = new System.Windows.Forms.ToolStripMenuItem();
			this.wRITEACCESSSETToolStripMenuItem = new System.Windows.Forms.ToolStripMenuItem();
			...
			base.SuspendLayout();
			this.button_Eth_Net_Selection.Location = new System.Drawing.Point(6, 32);
			this.button_Eth_Net_Selection.Name = "button_Eth_Net_Selection";
			this.button_Eth_Net_Selection.Size = new System.Drawing.Size(160, 29);
			...
			this.comboBox_net_ports.Location = new System.Drawing.Point(172, 32);
			this.comboBox_net_ports.Name = "comboBox_net_ports";
			this.comboBox_net_ports.Size = new System.Drawing.Size(379, 28);
			this.comboBox_net_ports.TabIndex = 1;
			this.comboBox_net_ports.SelectedIndexChanged += new System.EventHandler(comboBox_net_ports_SelectedIndexChanged);
			this.Log_textBox.Location = new System.Drawing.Point(12, 178);
			this.Log_textBox.Multiline = true;
			this.Log_textBox.Name = "Log_textBox";
			this.Log_textBox.ReadOnly = true;
			this.Log_textBox.ScrollBars = System.Windows.Forms.ScrollBars.Both;
			this.Log_textBox.Size = new System.Drawing.Size(539, 90);
			...
			this.button_Send_MAC_Frame.Location = new System.Drawing.Point(12, 116);
			this.button_Send_MAC_Frame.Name = "button_Send_MAC_Frame";
			this.button_Send_MAC_Frame.Size = new System.Drawing.Size(154, 29);
			...
			this.textBox_Send_Message.Location = new System.Drawing.Point(172, 115);
			this.textBox_Send_Message.Multiline = true;
			this.textBox_Send_Message.Name = "textBox_Send_Message";
			this.textBox_Send_Message.Size = new System.Drawing.Size(379, 61);
			...
			this.comboBox_OLT_stick_ADD.Location = new System.Drawing.Point(172, 66);
			this.comboBox_OLT_stick_ADD.Name = "comboBox_OLT_stick_ADD";
			this.comboBox_OLT_stick_ADD.Size = new System.Drawing.Size(379, 28);
			...
			this.dataGridView_OLT_Online_List.Location = new System.Drawing.Point(6, 297);
			...
			this.dataGridView_OLT_Online_List.Size = new System.Drawing.Size(1494, 397);
			...
			this.contextMenuStrip_OLT_Check.Items.AddRange(new System.Windows.Forms.ToolStripItem[7] { this.oNUStatusToolStripMenuItem, this.cPEWhiteListToolStripMenuItem, this.oLTSDKUpgradeToolStripMenuItem, this.oNUServicepushingToolStripMenuItem, this.oLTRemoveToolStripMenuItem, this.rEADACCESSSETToolStripMenuItem, this.wRITEACCESSSETToolStripMenuItem });
			this.contextMenuStrip_OLT_Check.Name = "contextMenuStrip_OLT_Check";
			this.contextMenuStrip_OLT_Check.Size = new System.Drawing.Size(300, 172);
			...
			this.pushing2OLTToolStripMenuItem.Size = new System.Drawing.Size(224, 26);
			...
			this.pollingToolStripMenuItem.Size = new System.Drawing.Size(224, 26);
			...
			this.rEADACCESSSETToolStripMenuItem.Name = "rEADACCESSSETToolStripMenuItem";
			this.rEADACCESSSETToolStripMenuItem.Size = new System.Drawing.Size(299, 24);
			this.rEADACCESSSETToolStripMenuItem.Text = "READ_ACCESS_SET";
			this.rEADACCESSSETToolStripMenuItem.Click += new System.EventHandler(rEADACCESSSETToolStripMenuItem_Click);
			this.wRITEACCESSSETToolStripMenuItem.Name = "wRITEACCESSSETToolStripMenuItem";
			this.wRITEACCESSSETToolStripMenuItem.Size = new System.Drawing.Size(299, 24);
			this.wRITEACCESSSETToolStripMenuItem.Text = "WRITE_ACCESS_SET";
			this.wRITEACCESSSETToolStripMenuItem.Click += new System.EventHandler(wRITEACCESSSETToolStripMenuItem_Click);
			...
			this.menuStrip1.Items.AddRange(new System.Windows.Forms.ToolStripItem[2] { this.oLT地址ToolStripMenuItem, this.cPEToolStripMenuItem });
			this.menuStrip1.Location = new System.Drawing.Point(0, 0);
			this.menuStrip1.Name = "menuStrip1";
			this.menuStrip1.Size = new System.Drawing.Size(1503, 28);
			this.menuStrip1.TabIndex = 8;
			this.menuStrip1.Text = "menuStrip1";
			this.textBox_LOG_Management.Location = new System.Drawing.Point(557, 31);
			...
			this.textBox_LOG_Management.Size = new System.Drawing.Size(943, 260);
			...
			this.OLT_On_Line_information_label.Location = new System.Drawing.Point(6, 274);
			...
			base.AutoScaleDimensions = new System.Drawing.SizeF(9f, 20f);
			base.AutoScaleMode = System.Windows.Forms.AutoScaleMode.Font;
			base.ClientSize = new System.Drawing.Size(1503, 696);
	...
	public struct Arp_Frame_Content
	{
		...
		{
			return $"SIP: {SenderProtocolAddress}, SMAC: {SenderHardwareAddress},DIP: {TargetProtocolAddress},{specified_code}";
		}
	}
	...
	public enum Command_Code
	{
		...
		OLT_Update_BIN_cmd = 68,
		OLT_login_send = 128,
		OLT_Status_report = 129,
		OLT_Alarm_report = 130,
		ONU_Status_report = 131,
		ONU_Alarm_report = 132
	}
	internal class MAC
	{
		...
		private ushort _destinationUDPport;
		...
		public MAC(string sourceMac, ushort MAC_Type, IPAddress sourceIP)
		{
			...
			ILiveDevice device = ((IEnumerable<ILiveDevice>)instance).FirstOrDefault((Func<ILiveDevice, bool>)((ILiveDevice d) => sourceMac2.Equals(((IPcapDevice)d).MacAddress?.ToString())));
			if (device == null)
			{
				MAC_init_Error = -3;
				return;
			}
			IInjectionDevice val = (IInjectionDevice)(object)device;
			if (val == null)
			{
				MAC_init_Error = -4;
				return;
			...
			_device = val;
			NetworkInterface networkInterface = NetworkInterface.GetAllNetworkInterfaces().FirstOrDefault((NetworkInterface n) => n.Description == ((IPcapDevice)device).Description);
		...
		public void StartListening()
		{
			...
			IInjectionDevice device = _device;
			ICaptureDevice val = (ICaptureDevice)(object)((device is ICaptureDevice) ? device : null);
			if (val != null)
			{
				((IPcapDevice)val).Open(new DeviceConfiguration
				{
					Mode = (DeviceModes)1,
					ReadTimeout = 100
				});
				...
				((IPcapDevice)val).Filter = text;
				val.OnPacketArrival += new PacketArrivalEventHandler(Device_OnPacketArrival);
				val.StartCapture();
			}
		}

		public int SendFrame(byte[] sn, byte[] password, byte[] payload, PhysicalAddress destMac, IPAddress destIP, ushort destUDPport = 64218)
		{
			ArgumentNullException.ThrowIfNull(payload, "payload");
			int result = 0;
			_destinationMac = destMac;
			_destinationIP = destIP;
			_destinationUDPport = destUDPport;
			try
			{
				byte[] array = new byte[4];
				if (payload.Length > 3)
				{
					Array.Copy(payload, array, 4);
				}
				else
				{
					Array.Copy(payload, array, payload.Length);
				}
				byte[] array2 = SN_Change.Hashcaculation(sn, password, array);
				byte[] array3 = new byte[array2.Length + payload.Length];
				Array.Copy(array2, array3, array2.Length);
				Array.Copy(payload, 0, array3, 4, payload.Length);
				lock (SendFrameLock)
				{
					Ether_Frame ether_Frame = new Ether_Frame(_destinationMac.GetAddressBytes(), _sourceMac.GetAddressBytes(), _MAC_Type, array3, _destinationIP, _sourceIP, _destinationUDPport);
					_device.SendPacket((ReadOnlySpan<byte>)ether_Frame.GetFrame(), (ICaptureHeader)null);
				}
			}
			catch
			{
		...
		private void Device_OnPacketArrival(object sender, PacketCapture e)
		{
			...
			RawCapture packet = ((PacketCapture)(ref e)).GetPacket();
			if (packet?.Data == null)
			{
				return;
			}
			Packet val = Packet.ParsePacket((LinkLayers)1, ((PacketCapture)(ref e)).GetPacket().Data);
			EthernetPacket val2 = (EthernetPacket)(object)((val is EthernetPacket) ? val : null);
			if (val2 != null)
			{
				Packet payloadPacket = ((Packet)val2).PayloadPacket;
				ArpPacket val3 = (ArpPacket)(object)((payloadPacket is ArpPacket) ? payloadPacket : null);
				if (val3 != null)
				{
					Arp_Frame_Content item = default(Arp_Frame_Content);
					item.SenderHardwareAddress = PhysicalAddress.Parse(val3.SenderHardwareAddress.ToString());
					item.SenderProtocolAddress = val3.SenderProtocolAddress;
					item.TargetHardwareAddress = PhysicalAddress.Parse(val3.TargetHardwareAddress.ToString());
					item.TargetProtocolAddress = val3.TargetProtocolAddress;
					item.Opcode = (ushort)(int)val3.Operation;
					byte[] array = new byte[19];
					try
					{
						Array.Copy(packet.Data, 42, array, 0, 19);
			...
			try
			{
				...
				if (array2[36] != 250 || array2[37] != 218)
				{
					return;
				}
				ushort num3 = 0;
		...
		public int BuildArpRequest(IPAddress destinationIp)
		{
			...
			try
			{
				...
				_device.SendPacket((ReadOnlySpan<byte>)array, (ICaptureHeader)null);
				return 0;
			}
			catch
			{
	...
	internal class Ether_Frame
	{
		...
		public Ether_Frame(byte[] destinationAddress, byte[] sourceAddress, ushort type, byte[] payload, IPAddress IP_Destination, IPAddress IP_Source, ushort dest_udp_port)
		{
			...
			Type = type;
			UdpPacketWrapper udpPacketWrapper = new UdpPacketWrapper(payload, 64218, dest_udp_port, IP_Destination, IP_Source);
	...
	public class PortReserver : IDisposable
	{
		...
		private int _reservedPort;

		private Thread _drainThread;

		private volatile bool _shouldDrain = true;

		private readonly object _lockObject = new object();

		public bool IsPortReserved => _isReserved;

		public int ReservedPort => _reservedPort;

		public string Status => _isReserved ? $"Port {_reservedPort} reserved (ICMP protection active)" : "Port not reserved";

		public bool ReservePort(int port, bool enableBackgroundDrain = true)
		{
			lock (_lockObject)
			{
				if (_isReserved)
				{
					Console.WriteLine($"Port {port} is already reserved");
					return true;
				}
				try
				{
					_udpSocket = new Socket(AddressFamily.InterNetwork, SocketType.Dgram, ProtocolType.Udp);
					_udpSocket.SetSocketOption(SocketOptionLevel.Socket, SocketOptionName.ReuseAddress, optionValue: true);
					_udpSocket.Blocking = false;
					_udpSocket.Bind(new IPEndPoint(IPAddress.Any, port));
					_reservedPort = port;
					_isReserved = true;
					Console.WriteLine($"Successfully reserved UDP port {port} for ICMP protection");
				...
				{
					Console.WriteLine($"Failed to reserve port {port}: {ex.SocketErrorCode}");
					Cleanup();
					return false;
				}
				catch (Exception ex2)
				{
					Console.WriteLine($"Unexpected error reserving port {port}: {ex2.Message}");
					Cleanup();
					return false;
				}
			}
		...
		{
			_shouldDrain = true;
			_drainThread = new Thread((ThreadStart)delegate
			{
				byte[] array = new byte[4096];
				int num = 0;
				Console.WriteLine("Starting background port drain thread...");
				while (_shouldDrain && _udpSocket != null)
				{
					try
					{
						if (_udpSocket.Poll(100000, SelectMode.SelectRead) && _udpSocket.Available > 0)
						{
							int num2 = _udpSocket.Receive(array, 0, array.Length, SocketFlags.None);
							if (num2 > 0)
							{
								Console.WriteLine($"Drained {num2} bytes from system socket (ICaptureDevice also received this)");
								num = 0;
							}
						}
						else
					...
					{
						Console.WriteLine(" Drain thread minor error: " + ex3.Message);
						Thread.Sleep(100);
					}
				}
				Console.WriteLine("Background port drain thread stopped");
			})
			{
				IsBackground = true,
				Name = $"PortDrainThread-{_reservedPort}"
			};
			_drainThread.Start();
		}
		...
		public void ReleasePort()
		{
			lock (_lockObject)
			{
				if (_isReserved)
				{
					Console.WriteLine($"\ud83d\udd13 Releasing UDP port {_reservedPort}");
					Cleanup();
				}
			}
		}
		...
		private void Cleanup()
		{
			...
			{
				Console.WriteLine("Error stopping drain thread: " + ex.Message);
			}
			try
			{
				_udpSocket?.Close();
			...
			{
				Console.WriteLine("Error closing socket: " + ex2.Message);
			}
			finally
			{
				_udpSocket = null;
	...
	public class SN_Change
	{
		...
		public static byte[] Hashcaculation(byte[] SN, byte[] password, byte[] first_4bytes)
		{
			uint num = 5381u;
			...
			{
				num = (num << 5) + num + password[j];
				num ^= num >> 13;
				num += num << 7;
				num ^= (uint)(password[j] << j % 4 * 8);
			}
			num ^= num >> 16;
			num += num << 3;
			num ^= num >> 4;
			for (int k = 0; k < first_4bytes.Length; k++)
			{
				num = (num << 5) + num + first_4bytes[k];
				num ^= num >> 13;
				num += num << 7;
				num ^= (uint)(first_4bytes[k] << k % 4 * 8);
			}
			num ^= num >> 16;
			num += num << 3;
			num ^= num >> 4;
	...
	public class OLT_share_data
	{
		...
		public ushort dest_UDP_port;

		public byte[] Vendor_Write_Key_byte;

		public byte[] Vendor_Read_Key_byte;

		public string sw_port;

		public int olt_read_access_tick;

		public int olt_write_access_tick;

		public int olt_read_access_flag;

		public int olt_write_access_flag;

		public double last_read_time;
		...
		public OLT_share_data()
		{
			...
			dest_UDP_port = 64218;
			sw_port = "";
			olt_read_access_tick = 0;
			olt_write_access_tick = 0;
			olt_read_access_flag = 0;
			olt_write_access_flag = 0;
			last_read_time = 0.0;
		}
	}
	...
	internal class ONU_ServiceType_GET
	{
		...
		public int send_package(MAC mAC)
		{
			...
			try
			{
				mAC.SendFrame(Encoding.UTF8.GetBytes(_share_data.OLT_SN), _share_data.Vendor_Read_Key_byte, uDP_Analysis_Package.content, _share_data.physicalAddress, _share_data.iPAddress, _share_data.dest_UDP_port);
						...
						case 1:
						{
							...
							for (int i = 0; i < num3; i++)
							{
								...
								for (int j = 0; j < 5; j++)
								{
									...
									oNU_Service_flow.max = (ushort)(content[4 + i * num4 + j * num5 + 1] * 256 + content[4 + i * num4 + j * num5 + 2]);
									oNU_Service_flow.fix = (ushort)(content[4 + i * num4 + j * num5 + 3] * 256 + content[4 + i * num4 + j * num5 + 4]);
									oNU_Service_flow.ass = (ushort)(content[4 + i * num4 + j * num5 + 5] * 256 + content[4 + i * num4 + j * num5 + 6]);
									oNU_Service_flow.type = content[4 + i * num4 + j * num5 + 7];
									oNU_Service_flow.priority = content[4 + i * num4 + j * num5 + 8];
									oNU_Service_flow.weight = content[4 + i * num4 + j * num5 + 9];
									oNU_Service_flow.valid = content[4 + i * num4 + j * num5 + 10];
									oNU_Service_flow.vlan1_id = (ushort)(content[4 + i * num4 + j * num5 + 11] * 256 + content[4 + i * num4 + j * num5 + 12]);
									oNU_Service_flow.vlan2_id = (ushort)(content[4 + i * num4 + j * num5 + 13] * 256 + content[4 + i * num4 + j * num5 + 14]);
									oNU_Service_flow.vlan3_id = (ushort)(content[4 + i * num4 + j * num5 + 15] * 256 + content[4 + i * num4 + j * num5 + 16]);
									oNU_Service_flow.vlan4_id = (ushort)(content[4 + i * num4 + j * num5 + 17] * 256 + content[4 + i * num4 + j * num5 + 18]);
									oNU_Service_flow.vlan5_id = (ushort)(content[4 + i * num4 + j * num5 + 19] * 256 + content[4 + i * num4 + j * num5 + 20]);
	...
	internal class ONU_Whitelst_GET
	{
		...
		public ONU_Whitelst_GET(OLT_share_data data)
		{
			...
			task_timing = new Package_Timing_Management(0.0, 0.0, 1.0, 20.0);
			_share_data = data;
			soure_MAC = PhysicalAddress.None;
			soure_IP = IPAddress.None;
			last_reading_package_time = 0.0;
	...
	internal class CPE_ServiceType_Set
	{
		...
		public int servicetype_push(MAC mAC)
		{
			...
			if (!oNU_Service_Type_Change_Notify_stack.IsEmpty && oNU_Service_Type_Change_Notify_stack.TryPop(out ONU_Service_Type_Change_Notify result2))
			{
				...
				for (int i = 0; i < count; i++)
				{
					array[i * 20 + 2] = (byte)(result2.oNU_Service_Type_Info.oNU_Service_Flows[i].max >> 8);
					array[i * 20 + 3] = (byte)(result2.oNU_Service_Type_Info.oNU_Service_Flows[i].max & 0xFFu);
					array[i * 20 + 4] = (byte)(result2.oNU_Service_Type_Info.oNU_Service_Flows[i].fix >> 8);
					array[i * 20 + 5] = (byte)(result2.oNU_Service_Type_Info.oNU_Service_Flows[i].fix & 0xFFu);
					array[i * 20 + 6] = (byte)(result2.oNU_Service_Type_Info.oNU_Service_Flows[i].ass >> 8);
					array[i * 20 + 7] = (byte)(result2.oNU_Service_Type_Info.oNU_Service_Flows[i].ass & 0xFFu);
					array[i * 20 + 8] = result2.oNU_Service_Type_Info.oNU_Service_Flows[i].type;
					array[i * 20 + 9] = result2.oNU_Service_Type_Info.oNU_Service_Flows[i].priority;
					array[i * 20 + 10] = result2.oNU_Service_Type_Info.oNU_Service_Flows[i].weight;
					array[i * 20 + 11] = result2.oNU_Service_Type_Info.oNU_Service_Flows[i].valid;
					array[i * 20 + 12] = (byte)(result2.oNU_Service_Type_Info.oNU_Service_Flows[i].vlan1_id >> 8);
					array[i * 20 + 13] = (byte)(result2.oNU_Service_Type_Info.oNU_Service_Flows[i].vlan1_id & 0xFFu);
					array[i * 20 + 14] = (byte)(result2.oNU_Service_Type_Info.oNU_Service_Flows[i].vlan2_id >> 8);
					array[i * 20 + 15] = (byte)(result2.oNU_Service_Type_Info.oNU_Service_Flows[i].vlan2_id & 0xFFu);
					array[i * 20 + 16] = (byte)(result2.oNU_Service_Type_Info.oNU_Service_Flows[i].vlan3_id >> 8);
					array[i * 20 + 17] = (byte)(result2.oNU_Service_Type_Info.oNU_Service_Flows[i].vlan3_id & 0xFFu);
					array[i * 20 + 18] = (byte)(result2.oNU_Service_Type_Info.oNU_Service_Flows[i].vlan4_id >> 8);
					array[i * 20 + 19] = (byte)(result2.oNU_Service_Type_Info.oNU_Service_Flows[i].vlan4_id & 0xFFu);
					array[i * 20 + 20] = (byte)(result2.oNU_Service_Type_Info.oNU_Service_Flows[i].vlan5_id >> 8);
					array[i * 20 + 21] = (byte)(result2.oNU_Service_Type_Info.oNU_Service_Flows[i].vlan5_id & 0xFFu);
				}
	...
	internal class In_band_FW_Upgrade
	{
		...
		public In_band_FW_Upgrade(OLT_share_data data)
		{
			...
			task_timing = new Package_Timing_Management(0.0, 0.0, 1.0, 10.0);
			_share_data = data;
			soure_MAC = PhysicalAddress.None;
			soure_IP = IPAddress.None;
			Frame_Sent_ID_Code = 0;
		...
		public int fw_download(MAC mAC)
		{
			...
			try
			{
				...
				for (num4 = 0; num4 < num3; num4++)
				{
					...
					{
						num7++;
						int num8 = send_package_FW(mAC, array, num5);
						...
						while (true)
						{
							num8 = analysis_package_OLT_Image_Send_cmd();
							Task.Delay(10);
							switch (num8)
							{
							case 0:
				...
				{
					int num9 = 0;
					int num10 = (num2 - (num + 12)) % num;
					byte[] array2 = new byte[num10];
					Array.Copy(bin_file, num6, array2, 0, num10);
	...
	internal class OLT_Stick_Management
	{
		...
		public OLT_Stick_Management(OLT_share_data oLT_Data, long instance)
		{
			...
			arp_timing = new Package_Timing_Management(0.0, 0.0, 1.0, 50.0);
...
namespace APP_OLT_Stick_V2
{
	...
	public class DatabaseHelper
	{
		...
		public void SaveOnuList(List<ONU_Performance_Notity> onuList)
		{
			...
			try
			{
				...
				try
				{
					...
					{
						SqlMapper.Execute((IDbConnection)val, text, (object)new
						{
						...
						}, (IDbTransaction)val2, (int?)null, (CommandType?)null);
					}
		...
		public void SaveOLTList(IEnumerable<OLT_Performance_Notity> oltList)
		{
			...
			try
			{
				...
				try
				{
					string text = "\r\n                        INSERT OR REPLACE INTO OLT_Performance \r\n                        (olt_ip, olt_sn, olt_mac, olt_status, olt_tx_pwr, olt_bias, olt_temperature, olt_rssi, alarm_status, olt_voltage, time_second, time_string,olt_pn,olt_swport)\r\n                        VALUES \r\n                        (@olt_ip, @olt_sn, @olt_mac, @olt_status, @olt_tx_pwr, @olt_bias, @olt_temperature, @olt_rssi, @alarm_status, @olt_voltage, @time_second, @time_string,@olt_pn,@olt_swport)";
		...
		public int sql_search(string query, out DataTable processTable)
		{
			...
			try
			{
				...
				try
				{
					...
					SQLiteCommand val2 = new SQLiteCommand(query, val);
					SQLiteDataAdapter val3 = new SQLiteDataAdapter(query, val);
					((DbDataAdapter)(object)val3).Fill(processTable);
					return 0;
				}
				finally
				{
	...
	internal class CPE_White_list_OP
	{
		...
		public ONU_Whitelst_Info file_parse()
		{
			try
			{
				using StreamReader streamReader = new StreamReader(filePath, detectEncodingFromByteOrderMarks: true);
			...
			try
			{
				...
				try
				{
					...
					for (int i = 0; i < oNU_Whitelst_Info.cPE_Struct.Count; i++)
					{
						...
						string text = $"insert into CPE_WHITE_LIST(CPE_SN,Service_Type,OLT_IP,ACTIVE,Comment)values ('{oNU_Whitelst_Info.cPE_Struct[i].SNStr}','{oNU_Whitelst_Info.cPE_Struct[i].service_type}','{oNU_Whitelst_Info.olt_ip}', '{value}','{oNU_Whitelst_Info.cPE_Struct[i].description}')";
						SQLiteCommand val2 = new SQLiteCommand(text, val);
						try
						{
	...
	public class ONU_Service_flow
	{
		...
		public ushort vlan1_id;

		public ushort vlan2_id;

		public ushort vlan3_id;

		public ushort vlan4_id;

		public ushort vlan5_id;
	}
	...
	internal class ONU_Service_Type_Parse
	{
		...
		public void file_parse()
		{
				...
				while ((text = streamReader.ReadLine()) != null)
				{
					...
					if (Regex.IsMatch(text2, pattern, RegexOptions.IgnoreCase))
					{
						...
						while ((text = streamReader.ReadLine()) != null && reading_error == 0)
						{
							...
							if (array[0].Contains("vlan1_id"))
							{
								oNU_Service_flow.vlan1_id = ushort.Parse(array[1]);
								continue;
							}
							if (array[0].Contains("vlan2_id"))
							{
								oNU_Service_flow.vlan2_id = ushort.Parse(array[1]);
								continue;
							}
							if (array[0].Contains("vlan3_id"))
							{
								oNU_Service_flow.vlan3_id = ushort.Parse(array[1]);
								continue;
							}
							if (array[0].Contains("vlan4_id"))
							{
								oNU_Service_flow.vlan4_id = ushort.Parse(array[1]);
								continue;
							}
							if (array[0].Contains("vlan5_id"))
							{
								oNU_Service_flow.vlan5_id = ushort.Parse(array[1]);
								continue;
							}
							reading_error = -1;
							break;
			...
			using StreamReader streamReader2 = new StreamReader(filePath, detectEncodingFromByteOrderMarks: true);
		...
		public int File_read()
		{
			...
			try
			{
				...
				try
				{
					int num = 0;
					((DbConnection)(object)val).Open();
					string text = $"insert into ONU_SERVICE_TYPE(Name,Service_Type_Number,Content,olt_ip)values ('{oNU_Service_Type_Info.ONU_Service_Type_Name}','{oNU_Service_Type_Info.ONU_Service_Type_No}','{content}', '{oNU_Service_Type_Info.olt_ip}')";
					SQLiteCommand val2 = new SQLiteCommand(text, val);
					try
					{
						if (((DbCommand)(object)val2).ExecuteNonQuery() == 0)
	...
	public class OLT_Performance_Notity : INotifyPropertyChanged
	{
		...
		private string _switch_port = string.Empty;

		private string _access_authority = string.Empty;

		private string _offline_issue = string.Empty;

		private string _olt_pn = string.Empty;

		public string olt_pn
		{
			get
			{
				return _olt_pn;
			...
			set
			{
				...
				{
					_olt_pn = value;
					OnPropertyChanged("olt_pn");
				}
			}
		}

		public string offline_issue
		{
			get
			{
				return _offline_issue;
			}
			set
			{
				if (_offline_issue != value)
				{
					_offline_issue = value;
					OnPropertyChanged("offline_issue");
				}
			}
		}
		...
		public string access_authority
		{
			get
			{
				return _access_authority;
			}
			set
			{
				if (_access_authority != value)
				{
					_access_authority = value;
					OnPropertyChanged("access_authority");
				}
			}
		}

		public string switch_port
		{
			get
			{
				return _switch_port;
			...
			set
			{
				if (_switch_port != value)
				{
					_switch_port = value;
					OnPropertyChanged("switch_port");
				}
			}
		}
		...
		public Bitmap StatusImage
		{
			get
			{
				string text = _olt_status;
				if (1 == 0)
				{
				}
				Bitmap result = text switch
				{
					"ON_LINE" => CreateColorBitmap(Color.Green, 16, 16), 
					"OFF_LINE" => CreateColorBitmap(Color.Red, 16, 16), 
					"FLASH" => CreateColorBitmap(Color.Yellow, 16, 16), 
					_ => CreateColorBitmap(Color.Gray, 16, 16), 
				};
				if (1 == 0)
				{
				}
		...
		private Bitmap CreateColorBitmap(Color color, int width, int height)
		{
	...
	public class ONU_HISTORY : Form
	{
		...
		private void InitializeComponent()
		{
			...
			((System.ComponentModel.ISupportInitialize)this.dataGridView_OLT_WL).BeginInit();
			base.SuspendLayout();
			this.button_Search_ONU_Performance.Location = new System.Drawing.Point(671, 27);
			...
			this.items_number.FormattingEnabled = true;
			this.items_number.Items.AddRange(new object[6] { "Last 100", "Last 200", "Last 500", "Last 1000", "Last 2000", "" });
			this.items_number.Location = new System.Drawing.Point(405, 28);
			this.items_number.Name = "items_number";
			this.items_number.Size = new System.Drawing.Size(109, 28);
			this.items_number.TabIndex = 21;
			this.label1.AutoSize = true;
			this.label1.ForeColor = System.Drawing.SystemColors.MenuHighlight;
			...
			this.comboBox_CPE_SN.Location = new System.Drawing.Point(520, 28);
			...
			this.button_Search_ONUSN_Performance.Location = new System.Drawing.Point(794, 26);
			...
			base.AutoScaleDimensions = new System.Drawing.SizeF(9f, 20f);
			base.AutoScaleMode = System.Windows.Forms.AutoScaleMode.Font;
			base.ClientSize = new System.Drawing.Size(947, 603);
